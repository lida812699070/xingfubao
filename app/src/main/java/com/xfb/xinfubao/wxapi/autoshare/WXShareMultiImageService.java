package com.xfb.xinfubao.wxapi.autoshare;
...
public class WXShareMultiImageService extends AccessibilityService {
    ...
    private void openDiscover(AccessibilityEvent event) {
        ...
        if (discoverNodeList == null || discoverNodeList.size() < 1) return;
        try {
    ...
    private void processingSnsTimeLineUI(AccessibilityEvent event) {
        ...
        // 过滤重复事件。
        if (prevSource == null || event.getSource() == null || event.getSource().equals(prevSource)) {
            return;
        }
    ...
    private void openAlbum(AccessibilityEvent event) {
        ...
        if (!albumNodeInfoList.isEmpty() && listView.getChild(1) != null) {
